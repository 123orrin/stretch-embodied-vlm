...
class FunmapNode(hm.HelloNode):
    ...
    def plan_a_path(self, end_xy_pix, robot_xya_pix=None, floor_mask=None): #### IMPORTANT? ####
    ...
    def reach_to_CG_target_callback(self, clicked_msg):
    ...
    def navigate_to_goal_topic_callback(self, goal_pose): #### IMPORTANT? ####
        self.logger.info(
    ...
    def navigate_to_goal_action_callback(self, goal_handle): #### IMPORTANT? ####
    ...
    def publish_map_to_odom_tf(self): #### IMPORTANT? ####
        with self.map_odom_tf_lock:
            self.tf2_broadcaster.sendTransform(
            create_map_to_odom_transform(self.map_to_odom_transform_mat, self.clock.now().to_msg()))
    ...
    def main(self):
        ...
        if self.map_filename != '':
            self.merged_map = ma.HeadScan.from_file(self.map_filename)  #### IMPORTANT FUNCTION FOR TAKING IN PRE-EXISTING MAPS ####
            # HeadScan.from_file(self.map_filename) takes in a .yaml file and loads it using yaml.FullLoader
            # This involves more functions under the hood but ultimately seems to return a list in the BaseConstructor.construct_document()
            # I recommend running FUNMAP using the filename for an existing map and printing the loaded yaml data to 
            # confirm that it's indeed in list format
            self.localized = False
        ...
        self.reach_to_CG_target_subscriber = self.create_subscription(
            PointStamped, '/CG_target', self.reach_to_CG_target_callback, 1, callback_group=self.callback_group)  # assume we have target point in PointStamped format from here

        self.tf2_broadcaster = tf2_ros.TransformBroadcaster(self)

        self.point_cloud_pub = self.create_publisher(
            PointCloud2, '/funmap/point_cloud2', 1) # want to replace with concept graphs point cloud, so maybe subscribe to topic on whc
...
## Run concept graphs to get target point coordinates
## In parallel (?), run FUNMAP demo to get map (MHI map) and navigate to target point given by concept graphs