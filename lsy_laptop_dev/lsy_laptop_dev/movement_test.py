...
class VLMTeleop(hm.HelloNode):
    ...
    def get_preprompt(self):
        ...
        query = 'You are given four categories: "move by specific distance", "describe", "chat", and “move to object”. From these four categories, output the one that best represents the prompt below. Make sure to use the exact same formatting as above, including keeping the words in lower case and maintaining all the spaces. In case of uncertainty, output "chat". In addition, output "True" or "False" if you think the prompt below requires you to receive an image to be able to best answer the prompt. If you do not have the information required to answer the prompt or if you do not have access to real-time data, you should output "True". You should also output “True” and the name of the object in lower case if the category is “move to object”. Your final output should be in the following format, substituting prompt_type for "move", "describe", "chat", and substituting use_image for "True", "False", and object_target for the name of the object: prompt_type&use_image&object_target \nPrompt: ' + user_prompt
    ...
    ############################################################# MOVE BY SPECIFIC DISTANCE #############################################################           
            ...
            ############ BASIC RESPONSE TO ACKNOWLEDGE COMMAND ############
            tts_response = self.openai_client.audio.speech.create(model=self.tts_model,
                                                      voice=self.tts_voice,
                                                      input='Sure!',
                                                      response_format="wav",
    ...
    ########################################################## MOVE TO OBJECT ##########################################################
        ...
        ############ STEP 1 ############
            ...
            # Get prompt in easy-to-use format from GPT-4o-mini:
            # self.vl_req.prompt = 'Reformat the following prompt into two lists with the following format:\nlist_1 = [direction_1, direction_2, ..., direction_i, ..., direction_n] | list_2 = [distance_1, distance_2, ..., distance_i, ..., distance_n]\ndirection_i must be one of four words: forward, backward, left, right. distance_i must be a float value. The unit of distance should be meters if direction[i] is forward or backward, and it should be radians if direction i is right or left. \nPrompt: ' + prompt
            self.vl_req.prompt = f'From the image, give directions in units of length for how the camera can arrive in front of the given object and face it. Be cautious with your commands and make sure to not collide with the object. Assume that the camera is facing forward, where forward is in the direction the camera is pointing. Make sure to number your commands. Try to use the least number of commands as possible. A sample output is “1. turn right by 20 degrees 2. Move forward by 2 meters” If you cannot see the object, output “None”. The object you are moving towards is: {object_target}'
        ...
        ############ STEP 2 ############
            self.vl_req.image = self.image
            self.vl_req.use_image = False
            self.vl_req.prompt = f'From the given instructions, reformat it into 2 lists: ‘list1’ and ‘list2’. In list1, add only one of the 4 following directions to each element: ‘forward’, ‘backward’, ‘left’, ‘right’. In list2, add the corresponding distance in meters or angle in radians. For ‘forward’ or ‘backwards’, add the distance in meters, and for ‘left’ or ‘right’, append the appropriate angle in radians. Format the output as follows: ‘list1 = [direction_1, direction_2, …, direction_n] | list2 = [distance/angle_1, distance/angle_2, …, distance/angle_n]’. Make sure to only return this format as the output. From the example instructions ‘Move forward by 1.5 meters’ would be ‘list1 = [forward] |list2 = [1.5]’. The given instructions are: {unformatted_response}'
            ...
            ############ STEP 4 ############
                ...
                print('EOIGAUFHOGISFHGPA;OFGN;DFGJHPURHP;JHJKDJBFJDHFBDSJHEGWUYP    QPPQERFBDJSBVXCJNVBSKDRHGPAIUHGBFAHJGFAIUAHflkjdshguye')
                print(move_directions_list)
            ...
            tts_response = self.openai_client.audio.speech.create(model=self.tts_model,
                                                        voice=self.tts_voice,
                                                        input=f'Sure, I will move to the {object_target}!',
                                                        response_format="wav",
                                                        timeout=60)
            audio_result_path = "./vlm_teleop_openai_tts.wav" # must be .WAV or .OGG
            tts_response.write_to_file(audio_result_path)
...
'''
    ...
    ## Node main
    def main(self):
        print('vfjaldskjfas;flkasng;lkafng;lanf')
        for i in range(1):
            print('qfsjgb,fdnafdlkjg;aflkg;mflgk;nfda')
            rclpy.spin_once(self)
            print('uishflgakjhldakjglfkj;zlkdjg;sfkg;sdlkf')
            point = JointTrajectoryPoint()
            point.time_from_start = Duration(seconds=0).to_msg()
            point = JointTrajectoryPoint()
            point.time_from_start = Duration(seconds=0).to_msg()
