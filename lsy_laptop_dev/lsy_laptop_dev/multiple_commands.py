...
class VLMTeleop(hm.HelloNode):
    ...
    def process_prompt(self, prompt_type, prompt):
        ...
        if prompt_type == 'describe' or prompt_type == 'chat':
            ...
            # move_distances_list = move_distances.split(', ')
            # final_dir_list = move_directions_list[0].split(',')
            # print(f'hsapdofimhsapoigmhfdoiugs,hfp,ijpsoihjpo {final_dir_list}')

            # assert len(move_directions_list) == len(move_distances_list)


            print('JFHGLDSMGJLFDMJG;ADLKBJDSKJB;NLKNZDKJFBLNZKJ NLDKJH;XZLKVN.Z,MFDNGKZCXHJB ;LXKJJNA.DNMF;')
            ...
            for i in range(len(move_directions_list)):
                ...
                # time.sleep(2)
                # print('iufaysoighmsaogimhpfdoibmhpfdoiuaphgoiahgpomdaimhfoidaugpoighm')
